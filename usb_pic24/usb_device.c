...
void __attribute__((interrupt,auto_psv,section(".ISR"))) _USB1Interrupt()
{
    BYTE i;
     ...
     */
     ...
     // Activity flag does NOT work if device is not in sleep (errata ...)
     // Damn ... how can they fuck so basic functionality ?!?!
     // Workaround: SOF interrupt will trigger as it's enabled
     // So if the USBactivityIE is enabled and we have a SOF interrupt, then we know we are back again ..
    if(USBSOFIF && USBActivityIE)
    {
