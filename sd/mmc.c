...
#define CS_LOW()  _LATB14 = 0	/* MMC CS = L */
#define CS_HIGH() _LATB14 = 1	/* MMC CS = H */
...
#define	FCLK_SLOW() do {SPI2STATbits.SPIEN = 0; SPI2CON1bits.SPRE = 5; SPI2CON1bits.PPRE = 1; SPI2STATbits.SPIEN = 1;} while(0)			/* Set slow clock (100k-400k) */ 
#define	FCLK_FAST()	do {SPI2STATbits.SPIEN = 0; SPI2CON1bits.SPRE = 6; SPI2CON1bits.PPRE = 3; SPI2STATbits.SPIEN = 1;} while(0) 		/* Set fast clock (depends on the CSD) */ // put to 8Mhz (SD spec min max freq 25Mhz ...)
...
#define rcvr_spi_m(p)	do { SPI2BUF = 0xFF; while (!SPI2STATbits.SPIRBF); *(p) = (BYTE)SPI2BUF; } while(0)
...
void power_on (void)
{
	...
	_TRISB14 = 0;
						/* Enable SPI2 */
	SPI2CON1 = 0x013B; // Fixme: frequency too high
	SPI2CON2 = 0x0000;
...
)
{
	...
	// Card detection: Use CS, sd card has a pull-up on this pin
	if (!_RB14) return Stat;	/* No card in the socket */
